...
def main(cfg):
    ...
    while should_keep_training:
        ...
        for data in tqdm(active_train_loader, dynamic_ncols=True, disable=not accelerator.is_main_process):
            ...
            accelerator.log({'train/loss': loss, 'train/learning_rate': optimizer.param_groups[0]['lr']}, total_step) # log the learning rate of DPT_params
            accelerator.log(metrics, total_step)

            ####visualize the depth_mono and disp_preds
            if total_step % 50 == 0 and accelerator.is_main_process:
